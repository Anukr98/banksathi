    ...
    <div className={styles.footer_wrapper}>
      <div className={styles.footer_containor}>
        <div className={styles.first_col}>
          ...
          <div className={styles.icon_wrap}>
            {SocialIcons?.map((icon, index) => {
              return (
                <div key={index.toString()} className={styles.circle_icon}>
                  {icon}
                </div>
              );
            })}
            ...
            {ImportantLinks.map((link, index) => {
              return (
                <li className={styles.item} key={index.toString()}>
                  <RiArrowRightSFill className="mr-2" />
                  {link}
                </li>
              );
            })}
        ...
        <div className={styles.third_col}>
          ...
          <p className={`${styles.para} pt-0`} style={{ paddingTop: "0" }}>
            (CIN: U72900KA2021PTC151652)
          </p>
          <p className={styles.para}>
            <MdLocationOn className="mr-4" size={"4rem"} />
            {strings.footer.third_col}
          </p>
